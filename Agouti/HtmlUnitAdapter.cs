...
namespace Agouti
{
    ...
    public class HtmlUnitAdapter : IBrowser
    {
        ...
        {
            _time = time;
            _browser = new WebClient(BrowserVersion.FIREFOX_3_6);
            _browser.setJavaScriptEnabled(true);
        }
        ...
        public void VisitAjax(string url)
        {
            _browser.setAjaxController(new NicelyResynchronizingAjaxController());
            Visit(url);
        }
        ...
        public int WaitForBackgroundJavaScript(int millisecondTimeout)
        {
            return _browser.waitForBackgroundJavaScript(millisecondTimeout);
        }
